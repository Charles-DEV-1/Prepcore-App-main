...
export type ProgressAnalytics = { trend: { score: number; completedAt: string }[]; accuracy: SubjectAccuracy[] };
...
export async function getProgressAnalytics(userId: string, examType: string): Promise<ProgressAnalytics> {
  const [{ data: sessions, error: sessionError }, { data: answers, error: answerError }] = await Promise.all([
    supabase.from('sessions').select('score,completed_at').eq('user_id', userId).eq('exam_type', examType.toLowerCase()).not('score', 'is', null).order('completed_at', { ascending: false }).limit(7),
